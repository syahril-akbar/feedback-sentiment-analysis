...
def main():
    ...
    with open(log_file_path, "w") as f:
        ...
        # [2] Tampilkan langkah-langkah Preprocessing (Contoh)
        print("--- Menampilkan Langkah-langkah Preprocessing (Contoh) ---")
        for i in range(3):
            contoh_teks = df["kritik dan saran"].iloc[i]
            preprocessing.tampilkan_langkah_preprocessing(contoh_teks)
        print("\n" + "="*50 + "\n")


        # [3] Lakukan preprocessing teks untuk seluruh dataset
        df["teks_bersih"] = df["kritik dan saran"].apply(preprocessing.proses_teks)

        # [4] Konversi teks ke TF-IDF
        tfidf_matrix, feature_names, vectorizer = tfidf_vectorizer.ubah_ke_tfidf(df["teks_bersih"])

        # [5] Lakukan clustering K-Means dengan pencarian k terbaik
        df["klaster"], best_k = clustering.kmeans_clustering(tfidf_matrix)

        # [6] Analisis sentimen menggunakan lexicon InSet
